                ...
                >
                  {statePartners
                    ...
                    .map((partner) => {
                      ...
                      const backendURL = ADMIN_URL;
                      let partnerImage;
                      if (
                        partner.profile_image &&
                        partner.profile_image.trim() !== ""
                      ) {
                        if (partner.profile_image.startsWith("http")) {
                          partnerImage = partner.profile_image;
                        } else {
                          // Clean path and ensure /storage/ prefix is used correctly
                          const cleanPath = partner.profile_image.replace(
                            /^\/+/,
                            "",
                          );
                          // Some paths might already have storage/ prefix, some might not
                          const finalPath = cleanPath.startsWith("storage/")
                            ? cleanPath
                            : `storage/${cleanPath}`;

                          partnerImage = `${backendURL.replace(/\/$/, "")}/${finalPath}`;
                        }
                      } else {
                        partnerImage =
